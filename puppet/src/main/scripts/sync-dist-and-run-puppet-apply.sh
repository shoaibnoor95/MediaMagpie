...
#
echo "** run puppet now..."
$CMD_SSH -l $USER $1 "sudo puppet apply --modulepath=$DIR_REMOTE_PUPPET/etc/puppet/modules/:/etc/puppet/modules --hiera_config=$DIR_REMOTE_PUPPET/etc/puppet/hiera.yaml $DIR_REMOTE_PUPPET/etc/puppet/manifests/site.pp"
EXIT=$?
