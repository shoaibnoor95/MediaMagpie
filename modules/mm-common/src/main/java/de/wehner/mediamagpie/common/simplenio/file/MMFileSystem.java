 ...
 * 
 * <p>
 * The default file system, obtained by invoking the {@link FileSystems#getDefault FileSystems.getDefault} method, provides access to the
 * file system that is accessible to the Java virtual machine. The {@link FileSystems} class defines methods to create file systems that
...
public abstract class MMFileSystem implements Closeable {
     ...
     * {@link WatchService watch-service}, and other closeable objects associated with this file system. The {@link FileSystems#getDefault
     ...
     * 
     * <p>
     * A file system provides access to a file store that may be composed of a number of distinct file hierarchies, each with its own
     * top-level root directory. Unless denied by the security manager, each element in the returned iterator corresponds to the root
     * directory of a distinct file hierarchy. The order of the elements is not defined. The file hierarchies may change during the lifetime
     * of the Java virtual machine. For example, in some implementations, the insertion of removable media may result in the creation of a
     * new file hierarchy with its own top-level directory.
     * 
     * <p>
     * When a security manager is installed, it is invoked to check access to the each root directory. If denied, the root directory is not
     * returned by the iterator. In the case of the default provider, the {@link SecurityManager#checkRead(String)} method is invoked to
     * check read access to each root directory. It is system dependent if the permission checks are done when the iterator is obtained or
     * during iteration.
     * 
     ...
     * 
     * <p>
     * The elements of the returned iterator are the {@link FileStore FileStores} for this file system. The order of the elements is not
     * defined and the file stores may change during the lifetime of the Java virtual machine. When an I/O error occurs, perhaps because a
     * file store is not accessible, then it is not returned by the iterator.
     * 
     * <p>
     * In the case of the default provider, and a security manager is installed, the security manager is invoked to check
     * {@link RuntimePermission}<tt>("getFileStoreAttributes")</tt>. If denied, then no file stores are returned by the iterator. In
     * addition, the security manager's {@link SecurityManager#checkRead(String)} method is invoked to check read access to the file store's
     * <em>top-most</em> directory. If denied, the file store is not returned by the iterator. It is system dependent if the permission
     * checks are done when the iterator is obtained or during iteration.
     * 
     * <p>
     * <b>Usage Example:</b> Suppose we want to print the space usage for all file stores:
     * 
     * <pre>
     ...
     * 
     * <p>
     * The {@link FileStore#supportsFileAttributeView(String) supportsFileAttributeView(String)} method may be used to test if an underlying
     * {@link FileStore} supports the file attributes identified by a file attribute view.
     * 
    ...
    /**
     * Returns a {@code PathMatcher} that performs match operations on the {@code String} representation of {@link Path} objects by
     * interpreting a given pattern.
     * 
     * The {@code syntaxAndPattern} parameter identifies the syntax and the pattern and takes the form: <blockquote>
     * 
     * <pre>
     ...
     * 
     * <p>
     * A {@code FileSystem} implementation supports the "{@code glob}" and "{@code regex}" syntaxes, and may support others. The value of
     * the syntax component is compared without regard to case.
     * 
     * <p>
     * When the syntax is "{@code glob}" then the {@code String} representation of the path is matched using a limited pattern language that
     * resembles regular expressions but with a simpler syntax. For example:
     * 
     * <blockquote>
     * <table border="0">
     * <tr>
     ...
     * <tr>
     * <td><tt>C:&#92;&#92;*</tt>
     * <td>Matches <tt>C:&#92;foo</tt> and <tt>C:&#92;bar</tt> on the Windows platform (note that the backslash is escaped; as a string
     * literal in the Java Language the pattern would be <tt>"C:&#92;&#92;&#92;&#92*"</tt>)</td>
     * </tr>
     * 
     * </table>
     * </blockquote>
     ...
     * <li>
     * <p>
     * The {@code *} character matches zero or more {@link Character characters} of a {@link Path#getName(int) name} component without
     ...
     * <li>
     * <p>
     * The backslash character ({@code \}) is used to escape characters that would otherwise be interpreted as special characters. The
     * expression {@code \\} matches a single backslash and "\{" matches a left brace for example.
     * </p>
     * </li>
     * 
     * <li>
     * <p>
     * The {@code [ ]} characters are a <i>bracket expression</i> that match a single character of a name component out of a set of
     * characters. For example, {@code [abc]} matches {@code "a"}, {@code "b"}, or {@code "c"}. The hyphen ({@code -}) may be used to
     * specify a range so {@code [a-z]} specifies a range that matches from {@code "a"} to {@code "z"} (inclusive). These forms can be mixed
     * so [abce-g] matches {@code "a"}, {@code "b"}, {@code "c"}, {@code "e"}, {@code "f"} or {@code "g"}. If the character after the
     * {@code [} is a {@code !} then it is used for negation so {@code [!a-c]} matches any character except {@code "a"}, {@code "b"}, or
     * {@code "c"}.
     * <p>
     * Within a bracket expression the {@code *}, {@code ?} and {@code \} characters match themselves. The ({@code -}) character matches
     * itself if it is the first character within the brackets, or the first character after the {@code !} if negating.
     * </p>
     * </li>
     * 
     * <li>
     * <p>
     * The {@code characters are a group of subpatterns, where the group matches if any subpattern in the group matches. The {@code ","}
     ...
     * <li>
     * <p>
     * Leading period<tt>&#47;</tt>dot characters in file name are treated as regular characters in match operations. For example, the
     * {@code "*"} glob pattern matches file name {@code ".login"}. The {@link Files#isHidden} method may be used to test whether a file is
     * considered hidden.
     * </p>
     * </li>
     * 
     ...
     * {@link MMFileSystem#getSeparator name-separators}.
     * </p>
     * </li>
     * 
     * <li>
     ...
     * 
     * <p>
     * When the syntax is "{@code regex}" then the pattern component is a regular expression as defined by the
    ...
    /**
     * Returns the {@code UserPrincipalLookupService} for this file system <i>(optional operation)</i>. The resulting lookup service may be
     * used to lookup user or group names.
     * 
     * <p>
     ...
     * 
     * @throws UnsupportedOperationException
     *             If this {@code FileSystem} does not support watching file system objects for changes and events. This exception is not
     *             thrown by {@code FileSystems} created by the default provider.
     * @throws IOException
     *             If an I/O error occurs
     */
