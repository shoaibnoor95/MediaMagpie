...
import de.wehner.mediamagpie.core.util.ArrayUtil;
...
public class MediaDao extends CreationDateBaseDao<Media> {
    ...
    @SuppressWarnings("unchecked")
    public List<Media> getAllByPathAndUri(User owner, String path, List<String> uris, Integer maxResults) {
    ...
    @SuppressWarnings("unchecked")
    public List<Media> getAllByPath(User owner, String path, Integer maxResults) {
        Criteria criteria = createCriteria();
        criteria.add(Restrictions.eq("_path", path));
        if (owner != null) {
    ...
    @SuppressWarnings("unchecked")
    public List<Media> getAllByCriterias(User owner, int start, int max, boolean ascending, SearchCriteriaCommand searchCriteria,
